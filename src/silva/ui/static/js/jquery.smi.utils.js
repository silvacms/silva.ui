  ...
  $.fn.updateTableColumnsWidths = function(options) {
    return this.each(function() {
      ...
      if (options.source) {
        ...
        $(this).find('tr:first').find('td').each(function(i) {
          if (options.fixedColumns[i]) {
            colWidths[i] = options.fixedColumns[i] + 'px';
      ...
      for (var i in colWidths) {
        if (typeof(colWidths[i]) == 'string') {
          newColWidth = colWidths[i];
        } else {
          newColWidth = Math.round(colWidths[i] / totalWidth * 100) + '%';
        }
